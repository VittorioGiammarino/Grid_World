...
class BatchBW(object):
    ...
    def pretrain_pi_hi(self, epochs, supervised_options):
        ...
        for t in range(epochs):
            ...
            accuracy = 1 - np.sum(np.abs(self.pi_hi(torch.FloatTensor(self.TrainingSet)).detach().numpy()-onehot_encoded_psi))/(self.option_dim*len(supervised_options))
    ...
    def pretrain_pi_b(self, epochs, Labels, option):
        ...
        for t in range(epochs):
            ...
            accuracy = 1 - np.sum(np.abs(self.pi_b[option](torch.FloatTensor(self.TrainingSet)).detach().numpy()-onehot_encoded_lab_b))/(self.option_dim*len(Labels))
        print(f"epoch {t}: accuracy {accuracy}")   
    ...
    def encode_state(self, state):      
        ...
        current_state_encoded = np.concatenate((coordinates,psi_encoded))
