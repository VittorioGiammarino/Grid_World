...
class Actor(nn.Module):
	...
	def forward(self, state):
		...
		a = F.relu(self.l2(a))
		return self.max_action[0] * torch.tanh(self.l3(a))
...
class Stochastic_Actor(nn.Module):
    ...
    def forward(self, state):        
        ...
        return torch.FloatTensor(self.max_action)*torch.tanh(distb.sample())
...
class TD3(object):
	def __init__(
		...
		self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), l_rate)

		self.critic = Critic(state_dim, action_dim).to(device)
		self.critic_target = copy.deepcopy(self.critic)
		self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), l_rate)
